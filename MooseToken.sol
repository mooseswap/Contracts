...
contract MooseToken is ERC20("Moose Finance", "MOOSE"), Ownable {
  ...
  function canUnlockAmount(address _account) public view returns (uint256) {
    // When block number less than startReleaseBlock, no MOOSEs can be unlocked
    if (block.number < startReleaseBlock) {
      return 0;
    }
    // When block number more than endReleaseBlock, all locked MOOSEs can be unlocked
    else if (block.number >= endReleaseBlock) {
      return _locks[_account];
    }
    // When block number is more than startReleaseBlock but less than endReleaseBlock,
    // some MOOSEs can be released
    else
    {
  ...
  function unlock() public {
    require(_locks[msg.sender] > 0, "no locked MOOSEs");
  ...
  // @dev move MOOSEs with its locked funds to another account
...
// FairLaunch is a smart contract for distributing MOOSE by asking user to stake the ERC20-based token.
contract FairLaunch is IFairLaunch, Ownable {
  ...
  struct UserInfo {
    ...
    //
    // We do some fancy math here. Basically, any point in time, the amount of MOOSEs
  ...
  struct PoolInfo {
    ...
    uint256 allocPoint; // How many allocation points assigned to this pool. MOOSEs to distribute per block.
    uint256 lastRewardBlock; // Last block number that MOOSEs distribution occurs.
    uint256 accMkatPerShare; // Accumulated MOOSEs per share, times 1e12. See below.
    uint256 accMkatPerShareTilBonusEnd; // Accumated MOOSEs per share until Bonus End.
  }

  // The Mkat TOKEN!
  MooseToken public moose;
  // Dev address.
  address public devaddr;
  // MOOSE tokens created per block.
  uint256 public moosePerBlock;
  // Bonus muliplier for early moose makers.
  uint256 public bonusMultiplier;
  // Block number when bonus MOOSE period ends.
  ...
  // The block number when MOOSE mining starts.
  ...
  */
  ...
  function setMkatPerBlock(uint256 _moosePerBlock) public onlyOwner {
    moosePerBlock = _moosePerBlock;
  }
  ...
  */
  ...
  // View function to see pending MOOSEs on frontend.
  function pendingMkat(uint256 _pid, address _user) external override view returns (uint256) {
    ...
    if (block.number > pool.lastRewardBlock && lpSupply != 0) {
      ...
      uint256 mooseReward = multiplier.mul(moosePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
      accMkatPerShare = accMkatPerShare.add(mooseReward.mul(1e12).div(lpSupply));
    }
  ...
  function updatePool(uint256 _pid) public override {
    ...
    uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
    uint256 mooseReward = multiplier.mul(moosePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    moose.mint(devaddr, mooseReward.div(10));
    moose.mint(address(this), mooseReward);
    pool.accMkatPerShare = pool.accMkatPerShare.add(mooseReward.mul(1e12).div(lpSupply));
    // update accMkatPerShareTilBonusEnd
    if (block.number <= bonusEndBlock) {
      moose.lock(devaddr, mooseReward.div(10).mul(bonusLockUpBps).div(10000));
      pool.accMkatPerShareTilBonusEnd = pool.accMkatPerShare;
    }
    if(block.number > bonusEndBlock && pool.lastRewardBlock < bonusEndBlock) {
      uint256 mooseBonusPortion = bonusEndBlock.sub(pool.lastRewardBlock).mul(bonusMultiplier).mul(moosePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
      moose.lock(devaddr, mooseBonusPortion.div(10).mul(bonusLockUpBps).div(10000));
      pool.accMkatPerShareTilBonusEnd = pool.accMkatPerShareTilBonusEnd.add(mooseBonusPortion.mul(1e12).div(lpSupply));
    }
    pool.lastRewardBlock = block.number;
  }

  // Deposit Staking tokens to FairLaunchToken for MOOSE allocation.
  ...
  // Harvest MOOSEs earn from the pool.
  ...
  function emergencyWithdraw(uint256 _pid) public {
    ...
    // Safe moose transfer function, just in case if rounding error causes pool to not have enough MOOSEs.
  function safeMkatTransfer(address _to, uint256 _amount) internal {
    uint256 mooseBal = moose.balanceOf(address(this));
    if (_amount > mooseBal) {
      moose.transfer(_to, mooseBal);
    } else {
